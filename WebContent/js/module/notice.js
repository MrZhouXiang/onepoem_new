...
var notice = (function($){
    ...
    var list =  function(){
        ...
        $.ajax({
            url:'/notice/doSearch/'+ currentPage,
            type:'post',
            dataType:'json',
            data:{
                keyword: keyword,
...
function noticeList(){
    ...
    $('#myModal').on('show.bs.modal', function (e) {
        ...
        $("#noticeAdd_dialog").load("/html/notice.html?t="+new Date().getTime()+" #noticeAdd",function(){
        	formValidate();
        	newNotice();
    ...
    $('#noticeEditModal').on('show.bs.modal', function (e) {
        ...
        $("#noticeEdit_dialog").load("/html/notice.html?t="+new Date().getTime()+" #noticeEdit",function(){
...
function renderTable(data){
    ...
    for (var i = 0; i < ths.length; i++) {
        ...
        if(type === "checkbox"){
            ...
            _input.setAttribute('data-id',data.notId);
            ...
            switch(type){
                case "status":
                    td.innerHTML = con == "new"?"<div class='sts-icon' title='未发送'></div>":"<div class='sts-icon active' title='已发送'></div>";
                    break;
                case "date":
                    var datestr = con;
...
//        content = content.replace(/\n/g,"<br/>")
        var cover = $("#fileName").val(); //从隐藏域中获取图片名称
        var orgid = $("#orgid").val();
