	...
	$.ajax({
        ...
        success:function(data){
            if (data.success) {
            	...
            	$("#photoCountVal").text(r.photoCount);
            	$("#transVal").text(r.trans);
            	$("#transFeeVal").text(r.transFee);
            	$("#totalFeeVal").text(r.totalFee);
            	$("#commentVal").text(r.comment);
            	$("#nameVal").text(a.name);
            	$("#telVal").text(a.tel);
	...
	$.ajax({
		...
		success:function(data){
			if(data.success){
				...
				$("#btn-activity").val(btnStatus);
				$("#btn-activity").attr("activity-status", data.status=="2"?"1":"2");
				
				//绑定事件
				 $("#btn-activity").on('click',function(){
					 	//var status = $("#activity-status").text();
			            layer.confirm('确定'+ btnStatus +'吗？', function(index){
			            	$.ajax({
			            		type:"post",
			            		dataType:"json",
			            		url:"/config/activity/updateStatus?status="+$("#btn-activity").attr("activity-status"),
			            		success:function(r)
			            		{
			            			layer.msg(r.success?"活动已"+btnStatus:btnStatus+"失败",2,{type:1,shade:false});
			            			initAct();
			            		}
