...
<script type="text/javascript">
	...
	function InitTable(pageIndex) {
		...
		currentLoc = 1;
		$.ajax({
					url : "webctrl/getAuthorList/" + currentPageVar,
					data : {
...
// 							显示列表
							for ( var i = 0; i < data.result.length; i++) {
								...
								$("#table_ads")
										.append(
												"<tr>"
														+ "<td>"
														+ data.result[i].dynasty
														+ "</td>"
														+ "<td>"
														+ data.result[i].name
														+ "</td>"
														+ "<td>"
														+ data.result[i].introduce.substr(0, 10)
														+ "..."
														+ "</td>"
														+ "<td>"
														+ "<a href='"+urlPath+data.result[i].url+"' target='_blank'><img width='150' height='150' src='"
															+urlPath+data.result[i].url+"' align='middle''></a>"
														+ "</td>" + "<td>" + "<a href='#' onclick=\"memberDetail_Click('" + data.result[i].id + "','" + data.result[i].dynasty_id + "')\">详情</a>"
														+ "</td>" + "</tr>");
							}
							//显示分布按钮							
	...
	function memberDetail_Click(id, author_id) {
		location.href = "authorDetail.jsp?id=" + id + "&dynasty_id=" + author_id;
	}
